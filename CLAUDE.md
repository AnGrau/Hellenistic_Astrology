...
1. **Géocodage lieu → latitude/longitude** — le CLI (jalon 5) exige aujourd'hui des coordonnées décimales déjà connues ; permettre de saisir directement une ville/un lieu de naissance en texte libre (ex. "Paris 14e, France") et résoudre automatiquement lat/lon. Points à trancher avant implémentation, pas à décider seul :
   - **Source de géocodage** : service externe (type Nominatim/OpenStreetMap) — dépendance réseau, et surtout question de confidentialité puisque cela enverrait le lieu de naissance d'un tiers à un service tiers (à mettre en regard des garde-fous sur les données personnelles ci-dessous) ; vs. base de données locale de villes — pas de fuite réseau mais plus lourde à maintenir et moins exhaustive.
   - **Désambiguïsation** : de nombreuses villes sont homonymes (plusieurs "Paris" dans le monde), donc il faut soit un indice supplémentaire (pays/région), soit une sélection interactive.
   - Recommandation par défaut : garder la saisie directe de lat/lon comme option toujours disponible (précision, hors-ligne, confidentialité), le géocodage n'étant qu'une alternative pratique.
...
10. **Skill Claude** (`skills/hellenistic-astrology/SKILL.md`) — wrapper fin autour du CLI (jalon 5, terminé), idéalement après que les Phases 2/3 existent au moins partiellement.
11. **Docker** — prévu ; sa condition initiale (un point d'entrée réel) est désormais remplie par le CLI du jalon 5, donc plus aucun blocage technique, mais toujours pas engagé sans décision explicite.
...
- Ne pas choisir seul l'ordre ou le périmètre des items listés dans "Reste à faire" sans validation explicite — la priorisation proposée est une recommandation, pas un mandat. En particulier, ne pas décider seul de l'approche pour le géocodage (item 1 : source externe vs locale, question de confidentialité) ni pour les Phases 2/3 (items 7/8) : ce sont des choix qui méritent leur propre discussion de planification.
